...
\end{equation}
where, exploiting the same notation \citet{Guiggiani2018}, $C_\alpha$ is the cornering stiffness of the front tire and $a_1$ is the distance between the Centre of Mass (CoM) and the front axle.
Tire slip angles $\alpha_{1i}$ and vehicle slip angles $\beta_{1i}$ are assumed equal in the same axle for each wheel $i=1,2$, hence $\alpha_{11} = \alpha_{12} = \alpha_{1}$ and  $\beta_{11} = \beta_{12} = \beta_{1}$.
%
Front wheel lateral forces $F_{y_{1i}}$ are estimated with a steady state assumption, leading to $F_{y_{11}} = F_{y_{12}} = F_{y}a_2/(2l)$.
$F_{y}$ summarise the total lateral force that acts on the vehicle model, $a_2$ is the distance between the CoM and the rear axle and $l$ is the wheelbase.
...
The point mass model dynamics is formulated in spatial domain instead of the time, and the \emph{direct collocation} is used to transform the \emph{Optimal Control Problem} (OCP) into a \emph{Non-Linear Program} (NLP). The NLP is coded in a scripting environment using the Matlab interface to the open-source CasADi framework written by \citet{Andersson2019}, which provides building
blocks to efficiently formulate and solve large-scale optimization problems, and solved through IPOPT~\cite{Wachter2006}.

The optimal control problem is formulated in order to find the optimal control sequence necessary to minimize the travel time of the next $l_N$ meters of the track.
...
\section{Conclusion}
The minimalistic MPC based on a point mass model with its body-fixed reference system is enough to race complex vehicles.
...
Nevertheless, the offset-free technique is a helpful and efficient method that lets model in a simple way some fundamental characteristics of the complex vehicle.
As a proof of concept, an internal model not aware of any aerodynamic drag proven to be improved by an estimation of the drag force through the disturbance state $d$.

Further modifies to the MPC internal model, as well as new approaches to extract the steer angle, can lead to drive the complex car even more on its limits.
